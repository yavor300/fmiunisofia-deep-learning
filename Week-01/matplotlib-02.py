...
def main():
    ...
    # Relationship answer:
    # Countries with larger populations do not necessarily live longer; at best
    # there is a weak positive trend with substantial spread at similar population sizes.
    plt.figure(figsize=(9, 5))
    plt.scatter(pop, life_exp, alpha=0.7)
    plt.xscale("log")
    plt.xlabel("Population [in millions]")
    plt.ylabel("Life Expectancy [in years]")
    plt.title("Population vs Life Expectancy (2007)")
    plt.tight_layout()
    plt.savefig(Path(__file__).with_name("matplotlib-02-population-vs-life-expectancy.png"), dpi=120)
    plt.close()
